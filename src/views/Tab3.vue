  ...
  setup() {
    ...
    const tab3Test = async (): Promise<boolean>  => {
      try {
        ...
        console.log('test isTable:(true)', isTable)
        isTable = await db.isTable('test2');
        console.log('test2 isTable:(false)', isTable)

        // sql example
        let sqlTestResult = await db.query("SELECT name FROM sqlite_master WHERE type='table' AND name='test'");
        console.log('sql example test isTable:(true)', sqlTestResult)

        let sqlTest2Result = await db.query("SELECT name FROM sqlite_master WHERE type='table' AND name='test2'");
        console.log('sql example test isTable:(false)', sqlTest2Result)
